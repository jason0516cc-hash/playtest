...
import { setHotbarSlideOffset } from './HotbarUI.js';
...
function runHomescreen() {
  ...
  // Dynamic import here (rather than a static import at the top of this file)
  // deliberately avoids a circular top-level dependency with main.js, which
  // itself calls into this file — a static cycle caused "Cannot access
  // before initialization" errors depending on which module loaded first.
  import('./main.js').then(({ startHomescreenHotbar }) => startHomescreenHotbar());
  ...
  if (!document.getElementById('_modeBarStyle')) {
    ...
    s.textContent = `
      ...
      .mode-btn:hover  { background: rgba(255,255,255,0.18); border-color: rgba(255,255,255,0.35); transform: scale(1.04); }
  ...
  function _modeBtn(label, onclick) {
  ...
  // ── Biome select bar (slides in above game title when Waves is clicked) ──────
  // Inject biome bar styles once
  if (!document.getElementById('_biomeBarStyle')) {
    ...
    bs.textContent = `
      #biome-select-bar {
        ...
        transition:
          opacity 0.22s ease,
          top 0.28s cubic-bezier(0.34,1.45,0.64,1);
        z-index: 9999;
      }
      #biome-select-bar.open {
        opacity: 1;
        pointer-events: auto;
        top: 18px;
      }
      ...
      .biome-btn:hover  { filter: brightness(1.18); border-color: rgba(255,255,255,0.45); transform: scale(1.05); }
      ...
      #biome-bar-close:hover { color: rgba(255,255,255,0.85); }
    `;
  ...
  const biomeClose = document.createElement('button');
  biomeClose.id = 'biome-bar-close';
  biomeClose.textContent = '✕';
  biomeClose.title = 'Close';

  biomeBar.appendChild(gardenBtn);
  biomeBar.appendChild(desertBtn);
  biomeBar.appendChild(oceanBtn);
  biomeBar.appendChild(biomeClose);
  document.body.appendChild(biomeBar);

  let biomeBarOpen = false;

  function _openBiomeBar() {
    if (biomeBarOpen) return;
    biomeBarOpen = true;
    requestAnimationFrame(() => biomeBar.classList.add('open'));
  }

  function _closeBiomeBar() {
    if (!biomeBarOpen) return;
    biomeBarOpen = false;
    biomeBar.classList.remove('open');
  }

  function _launchWaves(biome) {
    _closeBiomeBar();
    _closeModeBar();
    setHotbarSlideOffset(0);
    const v = nameInput.value.trim();
    player.name = v.length > 0 ? v : 'Unnamed';
    homeEl.style.display = 'none';
    if (typeof window.startWavesGame === 'function') window.startWavesGame(biome);
    runTransition(() => {});
  }

  gardenBtn.addEventListener('click', () => _launchWaves('garden'));
  desertBtn.addEventListener('click', () => _launchWaves('desert'));
  oceanBtn.addEventListener('click', () => _launchWaves('ocean'));
  // Expose so main.js can close the bar when returning to homescreen after death
  window._closeBiomeBar = _closeBiomeBar;

  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && biomeBarOpen) _closeBiomeBar();
  });
  ...
  // ── Hotbar slide animation when mode bar opens/closes ─────────────────────
  const HOTBAR_SLIDE_TARGET = 58; // px — how far hotbar drops when bar is open
  const HOTBAR_SLIDE_DUR    = 260; // ms — matches mode bar transition
  let _hotbarSlideRaf = null;
  let hotbarSlideOffset = 0;

  function _setAndTrack(v) { hotbarSlideOffset = v; setHotbarSlideOffset(v); }

  function _animateHotbarSlideTo(target) {
    if (_hotbarSlideRaf) { cancelAnimationFrame(_hotbarSlideRaf); _hotbarSlideRaf = null; }
    const from = hotbarSlideOffset;
    const startTime = performance.now();
    function step(now) {
      const t = Math.min(1, (now - startTime) / HOTBAR_SLIDE_DUR);
      const ease = 1 - Math.pow(1 - t, 3);
      _setAndTrack(from + (target - from) * ease);
      if (t < 1) { _hotbarSlideRaf = requestAnimationFrame(step); }
      else { _setAndTrack(target); _hotbarSlideRaf = null; }
    }
    _hotbarSlideRaf = requestAnimationFrame(step);
  }

  function _openModeBar() {
    if (modeBarOpen) return;
    modeBarOpen = true;
    requestAnimationFrame(() => modeBar.classList.add('open'));
    _animateHotbarSlideTo(HOTBAR_SLIDE_TARGET);
  }
